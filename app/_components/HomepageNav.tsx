  ...
  return (
    <nav className={overlay ? "absolute top-0 left-0 right-0 z-20 bg-black/40 backdrop-blur-sm" : "border-b border-[#2A2A2A] bg-[#0A0A0A]"}>
      <div className="max-w-5xl mx-auto px-6 sm:px-8 py-4 flex items-center justify-between">
        <Link href="/" className={`text-xs sm:text-sm font-black tracking-widest uppercase transition-colors whitespace-nowrap ${overlay ? "text-white/90 hover:text-white" : "text-[#6A6A6A] hover:text-[#AAFF00]"}`}>
          Experiences | Curated
        </Link>
        <div className="flex items-center gap-6">
          ...
          <Link href="/trip-board" className={`text-sm transition-colors whitespace-nowrap ${overlay ? "text-white/80 hover:text-white" : "text-[#6A6A6A] hover:text-[#AAFF00]"}`}>
            Trip Board
          </Link>
          {email && (
            <Link href="/my-travels" className={`hidden sm:block text-sm transition-colors ${overlay ? "text-white/80 hover:text-white" : "text-[#6A6A6A] hover:text-[#AAFF00]"}`}>
              My Travels
            </Link>
          )}
          {email ? (
