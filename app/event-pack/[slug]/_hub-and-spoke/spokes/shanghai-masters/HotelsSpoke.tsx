...
export default async function HotelsSpoke({ eventSlug }: { eventSlug: string }) {
  ...
  return (
    <SpokeShell
      ...
      ctaCopy="Buy the Where to Stay Guide alone, or the full Event Pack, to unlock our hotel recommendation, real 4-star pricing, the Golden Week booking-timing trap, and our verdict on which area to actually book for your trip."
    >
      {/* Free teaser — the central-vs-near-venue tradeoff framed, no hotel
          card, no pricing, no Golden Week timing detail. Everything else in
          this spoke is gated behind isUnlocked (full pack or the Where to
          Stay Guide mini-pack) — whole-spoke gating per the mini-packs
          pilot design, replacing the previous "free factual content +
          gated verdict" split this spoke used before. */}
      ...
      {/* !isUnlocked gate added 15 Sep 2026 — redundant once the full
          hotel guide card below is visible, and the founder wants a
          purchaser's page to match production exactly with no added
          content once unlocked. */}
      {!isUnlocked && (
        <div className="flex flex-col gap-2 mb-8">
          <p className="text-sm text-[#A3A3A3]">
            <span className="text-white font-bold">Central Shanghai:</span>{" "}near Hongqiao Railway Station, best if a Suzhou or Hangzhou day trip is part of your plan
          </p>
          <p className="text-sm text-[#A3A3A3]">
            <span className="text-white font-bold">Near-venue Minhang:</span>{" "}cuts daily shuttle time on a tennis-only trip, at real savings over central rates
          </p>
        </div>
      )}
