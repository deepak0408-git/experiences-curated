...
export default async function TicketsSpoke({ eventSlug }: { eventSlug: string }) {
  ...
  return (
    <SpokeShell
      ...
      ctaCopy="Buy the Ticket Guide alone, or the full Event Pack, to unlock real 2026 pricing for every stand, the full side-by-side comparison, where to actually buy, and our verdict on which stand we'd pick for a first Singapore GP."
    >
      ...
      <p className="text-sm text-[#A3A3A3] leading-7 mb-8">
        Sixteen named grandstands and two Walkabout tiers span Marina Bay Street Circuit. They&apos;re built around
        genuinely different ideas of what&apos;s worth watching, not just different price points for the same
        experience.
      </p>

      {/* !isUnlocked gate added 15 Sep 2026 — redundant once the full
          comparison table below is visible, and the founder wants a
          purchaser's page to match production exactly with no added
          content once unlocked. */}
      {!isUnlocked && (
        <div className="flex flex-col gap-2 mb-8">
          {STANDS.map((s) => (
            <p key={s.name} className="text-sm text-[#A3A3A3]">
