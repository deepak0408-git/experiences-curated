...
// Modeled directly on scripts/archive-wimbledon-2026.mjs (the real reference
// implementation, per the migrate-from-classic-to-hub-spoke skill §1 step 3).
...
// One-off archive snapshot (migrate-from-classic-to-hub-spoke skill §1 step 3),
// run manually right before Italian GP's sportingEvents row is renamed to the
// evergreen slug and rolled forward to its 2027 edition. Same
// SECTION_MAP/SECTION_ORDER, same PACK_EDITORIAL/TOURNAMENT_RHYTHM source
// keyed on the OLD "italian-gp-2026" slug (still intact in
// lib/pack-content.ts, preserved deliberately). Captures the classic-pack
// edition, since that's the real format 2026 buyers experienced —
// packFormat has not gone live as hub_and_spoke for any real purchaser as of
// this snapshot.
...
const ARCHIVE_SLUG = "italian-gp-2026"; // the real, dated slug this edition traded under — the PACK_EDITORIAL/TOURNAMENT_RHYTHM key
...
const [event] = await db.select().from(sportingEvents).where(eq(sportingEvents.id, ITALIAN_GP_ID));
if (!event) throw new Error("Italian GP event row not found");
if (event.slug !== "italian-gp-2026") {
  console.warn(`⚠ Current live slug is "${event.slug}", not "italian-gp-2026" — continuing anyway, archive is keyed by ID + seasonYear, not slug.`);
}
...
const pdfKey = `sporting-events/archive/italian-gp-${SEASON_YEAR}.pdf`;
...
const jsonPayload = {
  ...
  pricing: {
    note: "Real display prices at time of archive — see PACK_PRICING (classic, lib/packPricing.ts) for italian-gp-2026",
    earlyBirdDisplay: event.earlyBirdDisplay,
    standardDisplay: event.standardDisplay,
  },
...
const jsonKey = `sporting-events/archive/italian-gp-${SEASON_YEAR}.json`;
...
writeFileSync(`scratch-italian-gp-${SEASON_YEAR}-archive.json`, jsonBuffer);
