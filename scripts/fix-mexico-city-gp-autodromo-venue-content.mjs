...
// Rebuilds "Autódromo Hermanos Rodríguez — Inside the Venue" per founder
// request 6 Sep 2026: cut the Rodríguez brothers history to one paragraph
// (was 2 full paragraphs of pure history out of 4 total) and replace the
// freed-up space with real, sourced venue/facility/zone detail that was
// previously missing — circuit layout (4.304km, 17 corners, 1.29km main
// straight), Foro Sol Norte/Sur stadium seating breakdown, the Pit Building
// (33 team garages, Paddock Club positioned directly above it), the
// ~470-capacity Media Center, and the wider ~292-hectare Magdalena Mixhuca
// complex context. See scripts/seed-mexico-gp-autodromo-venue.mjs for the
// corrected source script and full sourcing in its editorialNote.
...
const insiderTips = [
  "The venue has 14 access gates total, and the official guidance is to enter through whichever one sits closest to your actual grandstand rather than the first gate you come across — the circuit is large enough that entering at the wrong end adds a genuinely long walk, not just a few extra minutes.",
